...
import { getToken, clearToken, clearStoredUser } from '../lib/sessionStorage';
...
// Nota FSD: este cliente vive en `shared`, la capa más baja — por eso lee el token
// directamente de `shared/lib/sessionStorage` en vez de depender de `entities/session`
// (useAuth). En un 401 crítico, limpia la sesión y redirige él mismo; el redirect fuerza
// una recarga completa, así que el estado de React de `entities/session` no necesita
// enterarse — al recargar, useAuth vuelve a leer sessionStorage y ve que no hay token.
